...
function update(){
	...
	if (hhogs==0&&cash<400||hhogs==1&&cash<200){
		up++;
...
function sellIncrease(){
	if(cash>=sCost){
		cash-=sCost;
		var increase=sellAmount*percentIncrease;
		sellAmount+=Math.floor(increase);
		buyAmount+=Math.floor(increase);
		...
		document.getElementById("buy1").innerHTML="$"+buyAmount;
		document.getElementById("buy10").innerHTML="$"+(buyAmount*10);
	}
	else{error(1)}
}
...
function buyHHogs(num){
	if(cash>=num*buyAmount&&hhogs<maxHogs){
		cash-=num*buyAmount;
		hhogs+=num;
		if(hhogs>maxHogs){
			hhogs=maxHogs;
...
/* This function allows you to buy hedgehogs if you meet the minimum requirements */
